...
- **商品检测**：自动识别商品（衣服、鞋子、包等），提供 eBay 检索服务
...
2. **推理阶段**：点击物体，AI 生成操作意图（混合编辑、信息、导航、搜索）
3. **执行阶段**：选择意图，执行相应操作
   - **图像编辑**：使用 Gemini 进行创意编辑
   - **信息查询**：显示物体相关信息
   - **网页导航**：打开相关链接
   - **商品搜索**：对于商品自动提供 eBay 检索服务
...
- SERP API Key（可选，用于网络搜索功能）

### 部署选项
- **开发环境**：本地运行前后端
- **生产环境**：推荐使用 Railway（后端）+ Vercel（前端）
- **详细部署指南**：查看 [DEPLOYMENT.md](./DEPLOYMENT.md) 和 [RAILWAY_SETUP.md](./RAILWAY_SETUP.md)
...
### 智能功能推荐
- **用户意图优先**：从用户角度思考"点击这个物体时想要什么"，再提供相应功能
- **商品识别**：自动识别商品（衣服、鞋子、包等），基于用户购买意图自然提供 eBay 检索服务
- **创意与实用平衡**：识别用户创意探索意图，提供改色、换风格等编辑选项
- **多类型意图**：基于真实用户意图，混合图像编辑、信息查询、网页导航、商品搜索等多种操作类型
...
- [x] **多类型意图系统**：支持图像编辑、信息查询、网页导航、搜索功能（详见 [多类型意图文档](./backend/MULTI_ACTION_INTENTS.md)）
- [x] **SERP API 集成**：自动利用互联网资源生成功能建议（详见 [SERP 集成文档](./backend/SERP_INTEGRATION.md)）
- [ ] 使用 Redis 替代内存缓存
- [ ] 添加 Session 管理
- [ ] 支持多图片上传
- [ ] 添加撤销/重做功能
...
- ✅ **多类型意图系统**：支持图像编辑、信息查询、网页导航、搜索四种操作类型
- ✅ **SERP API 集成**：自动利用互联网资源生成更丰富的功能建议
